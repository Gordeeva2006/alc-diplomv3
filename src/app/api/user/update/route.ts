...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.info(`${LOG_PREFIX} [UPDATE_USER]`, { email: data.email, phone: data.phone })
      ...
      if (existingClient) {
        console.info(`${LOG_PREFIX} [UPDATE_CLIENT]`, {
        ...
        if (data.client.type === "individual") {
          if (existingClient.type !== "individual") {
            console.info(`${LOG_PREFIX} [DELETE_LEGAL_ENTITY]`, existingClient.id)
            await connection.query("DELETE FROM legal_entities WHERE client_id = ?", [existingClient.id])
          }

          console.info(`${LOG_PREFIX} [UPSERT_INDIVIDUAL]`, data.client.individual)
          await connection.query(
          ...
          if (existingClient.type !== "legal_entity") {
            console.info(`${LOG_PREFIX} [DELETE_INDIVIDUAL]`, existingClient.id)
            await connection.query("DELETE FROM individuals WHERE client_id = ?", [existingClient.id])
          }

          console.info(`${LOG_PREFIX} [UPSERT_LEGAL_ENTITY]`, data.client.legalEntity)
          await connection.query(
        ...
        console.info(`${LOG_PREFIX} [CREATE_CLIENT]`, data.client)
        const [clientResult] = await connection.query<{ insertId: number }>(
        ...
        if (data.client.type === "individual") {
          console.info(`${LOG_PREFIX} [CREATE_INDIVIDUAL]`, data.client.individual)
          await connection.query(
            "INSERT INTO individuals (client_id, inn, company_name, ogrnip) VALUES (?, ?, ?, ?)",
            [
              clientId,
          ...
          console.info(`${LOG_PREFIX} [CREATE_LEGAL_ENTITY]`, data.client.legalEntity)
          await connection.query(
            "INSERT INTO legal_entities (client_id, inn, company_name, kpp, ogrn) VALUES (?, ?, ?, ?, ?)",
            [
              clientId,
