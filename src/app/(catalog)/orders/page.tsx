...
export default function OrdersPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-grow w-7xl mx-auto mt-16">
      ...
      {modalOrder && (
        <div className="fixed inset-0 bg-black text-white bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-dark p-6 rounded-lg shadow-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Детали заказа #{modalOrder.id}</h2>
              <button
                onClick={closeModal}
            ...
            <div className="mb-4">
              <p><strong>Дата:</strong> {new Date(modalOrder.created_at).toLocaleString()}</p>
              <p><strong>Юридический адрес:</strong> {modalOrder.legal_address}</p>
            </div>
            <h3 className="text-lg font-semibold mb-2">Файлы</h3>
            <div className="mb-4">
            ...
            <table className="w-full mb-4">
              <thead>
                <tr className="border-b border-gray-300">
                  ...
                  <th className="py-2 text-left">Упаковка</th>
                  <th className="py-2 text-left">Кол-во</th>
                  <th className="py-2 text-left">Цена за ед.</th>
                  <th className="py-2 text-left">Итого</th>
                </tr>
              </thead>
              <tbody>
                {modalOrder.items.map((item, index) => (
                  <tr key={index} className="border-b border-gray-200">
                    ...
                    <td className="py-2">{item.packaging_name || "Не указана"}</td>
              ...
              <tfoot>
                <tr className="border-t border-gray-300">
                  <td colSpan={4} className="py-2 text-right">Общая сумма:</td>
                  <td className="py-2 font-bold">{Number(modalOrder.total_amount).toFixed(2)} ₽</td>
                </tr>
              </tfoot>
            </table>
            <div className="flex justify-end">
