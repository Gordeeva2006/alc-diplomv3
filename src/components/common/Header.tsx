...
export default function Header() {
  ...
  const getIconForMenuItem = (iconName: string) => {
    switch (iconName) {
      ...
      case "FaUserShield":
        return <FaUserShield className="min-w-[24px]" />;
      default:
        return null;
    }
  };
  ...
  const filteredMenuItems = HEADER_LINKS.menuItems
    .filter((item: MenuItem) => {
      ...
      if (item.roles && session?.user?.role) return item.roles.includes(session.user.role);
      return true;
    });
  ...
  const handleAuthAction = () => {
    if (status === "authenticated") signOut({ callbackUrl: HEADER_LINKS.home });
  };
  ...
  return (
    <div>
      <header className="bg-dark fixed top-0 left-0 right-0 z-50 shadow-lg">
        {/* Основная панель */}
        <div className="max-w-7xl mx-auto flex items-center justify-between h-16  lg:px-4">
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-6">
            {status === "authenticated" && (
              <div className="text-white text-sm flex items-center gap-2">
                ...
                <span className={`${
                  session.user?.role === "director" ? "text-green-400" : 
                  session.user?.role === "admin" ? "text-red-400" : 
                  session.user?.role === "manager" ? "text-blue-400" :
                  session.user?.role === "stmanager" ? "text-purple-400" :
                  "text-green-400"
                } font-medium truncate max-w-[120px] xl:max-w-none`}>
                  {session.user?.role === "director" ? "Директор" :
                   session.user?.role === "admin" ? "Администратор" :
                   session.user?.role === "manager" ? "Менеджер" :
                   session.user?.role === "stmanager" ? "Ст. менеджер" :
                   ""}
            ...
            {status === "authenticated" ? (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 lg:px-4 lg:py-2 bg-red-600 hover:bg-red-700 text-white rounded transition-colors"
                aria-label="Выход"
              >
                <FaSignOutAlt className="text-lg flex-shrink-0" />
                <span className="hidden lg:inline text-sm xl:text-base">Выход</span>
              </button>
            ) : (
              <button
                onClick={handleAuthAction}
        ...
        <div className={`md:hidden fixed inset-0 z-40 bg-black/50 transition-opacity duration-300 ${
          ...
          <div className={`absolute top-16 left-0 right-0 bg-dark shadow-lg transform transition-transform duration-300 ${
