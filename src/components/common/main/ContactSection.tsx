...
const ContactSection = () => {
  ...
  const formRef = useRef<HTMLFormElement | null>(null); // <-- Используем ref для формы
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Безопасный сброс формы через ref
      if (formRef.current) {
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            <form 
              ...
              ref={formRef} // <-- Привязываем ref к форме
              className="flex flex-col justify-around bg-dark rounded-2xl p-4 shadow-md h-full"
            >
              {/* Поле имени */}
              <input 
                type="text" 
              ...
              <div className="space-y-4 mt-2">
                ...
                {message && (
                  <p className={`text-center ${message.includes('успешно') ? 'text-green-500' : 'text-red-500'}`}>
                    {message}
                  </p>
                )}
              </div>
